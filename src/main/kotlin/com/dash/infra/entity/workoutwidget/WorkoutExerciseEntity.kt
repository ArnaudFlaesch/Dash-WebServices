...
@SqlResultSetMappings(
    SqlResultSetMapping(
        name = "workoutStatsByYear", classes = [ConstructorResult(
            targetClass = WorkoutStatsByYearEntity::class,
            columns = [ColumnResult(name = "workoutTypeId"), ColumnResult(name = "totalNumberOfReps"), ColumnResult(name = "workoutTypeName"), ColumnResult(name = "monthPeriod"), ColumnResult(
                name = "yearPeriod"
            )]
        )]
    ), SqlResultSetMapping(
        name = "workoutStatsByInterval", classes = [ConstructorResult(
            targetClass = WorkoutStatsByIntervalEntity::class, columns = [ColumnResult(name = "totalNumberOfReps"), ColumnResult(name = "workoutTypeName")]
        )]
    )
)
@Entity
...
data class WorkoutExerciseEntity(
    @EmbeddedId var workoutExerciseId: WorkoutExerciseEntityId, var numberOfReps: Int = 0,

    @ManyToOne @JoinColumn(name = "workout_session_id", insertable = false, updatable = false) var workoutSession: WorkoutSessionEntity? = null,

    @ManyToOne @JoinColumn(name = "workout_type_id", insertable = false, updatable = false) var workoutType: WorkoutTypeEntity? = null
) {
    fun toDomain(): WorkoutExerciseDomain = WorkoutExerciseDomain(
        workoutSessionId = this.workoutExerciseId.workoutSessionId, workoutTypeId = this.workoutExerciseId.workoutTypeId, numberOfReps = this.numberOfReps
    )
}
...
data class WorkoutExerciseEntityId(
    ...
    @Column(name = "workout_session_id") val workoutSessionId: Int,

    @Column(name = "workout_type_id") val workoutTypeId: Int
) : Serializable {
    companion object {
